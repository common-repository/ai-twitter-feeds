...
function ai_option_page(){ ?>
	<div class="wrap"> 
		...
		<div style='background-color: #ddd; padding: 5px;'><p><?php _e('Copy and paste this shortcode directly into the page, post or widget where you\'d like to display the feed:','aitwitterfeeds');?></p>
		<p style="color: red; font-weight:bold;"><?php _e("[AIGetTwitterFeeds ai_username='Your Twitter Name(Without the '@' symbol)' ai_numberoftweets='Number Of Tweets' ai_tweet_title='Your Title' ai_load_more_text='Load More...' ai_list_grid='true']",'aitwitterfeeds');?></p>
		<p><?php _e('<div style="padding: 5px;  color: red;">All the options along with the shortcode are optional.</div>','aitwitterfeeds');?></p></div>
				...
				<tr class="odd" valign="top">
					<th scope="row">
						<label for="ai_display_number_of_tweets">
							<?php _e('Number Of Tweets Per Page:','aitwitterfeeds');?>
						</label>
					</th>
					<td>
						<input type="text" id="ai_display_number_of_tweets" name="ai_display_number_of_tweets" class="regular-text" value="<?php echo esc_attr(get_option('ai_display_number_of_tweets')); ?>" required />
						<p></p>
					</td>
				</tr>	
				<tr class="even" valign="top">
					<th scope="row">
						<label for="ai_load_more_text">
							<?php _e('Load More Button Text:','aitwitterfeeds');?>
						</label>
					</th>
					<td>
						<input type="text" id="ai_load_more_text" name="ai_load_more_text" class="regular-text" value="<?php echo esc_attr(get_option('ai_load_more_text')); ?>" />
						<p></p>
					</td>
				</tr>
                <tr class="odd" valign="top">
                    ...
                    <td>
                        <?php 
                        $ai_list_grid = esc_attr(get_option('ai_list_grid')); 
                        $checked = "";
                        if($ai_list_grid)
                        {
                            $checked = "checked"; 
                        ...
                        ?>
                        <input type="checkbox" <?php echo $checked; ?> id="ai_list_grid" name="ai_list_grid" class="regular-text" value="true" />
                        <label for="ai_list_grid">
                            <?php _e('Grid View For Shortcode','aitwitterfeeds');?>
                        </label>
                        <p></p>
                    </td>
                </tr>
...
function ai_get_twitter_feeds($atts){
	...
	if($ai_twitteruser!='' && $ai_notweets !='' && $ai_consumerkey!='' && $ai_consumersecret!='' && $ai_accesstoken!='' && $ai_accesstokensecret!='') {
		...
		$ai_output="<div class='ai-tweets'>
		<div class='".$ai_class." ai-tweets-listing'>
		".$ai_wid_title."					
		...
		$ai_output .= "<input type='hidden' class='ai_numberoftweets' value='".$ai_notweets."'>";
		$ai_output .= "<input type='hidden' class='ai_twitteruser' value='".$ai_twitteruser."'>";
		$ai_output .= "<div class='ai-twitter-list-wrap'><div class='ai-twitter-list ".$grid_class."'>";
		for($i=0; $i<count($ai_tweets); $i++) {
			if(!empty($ai_tweets->errors)) {
				...
				</span>&nbsp;<span class="tweet_author"><a href="https://twitter.com/'.$ai_twitteruser.'" target="_blank">@'.$ai_twitteruser.'</a></span>';
				...
				$ai_replay_html='<a target="_blank" class="tweet-reply" href="https://twitter.com/intent/tweet?in_reply_to='.$ai_tweets[$i]->id_str.'">reply</a>';
				
				$ai_retweet_html='<a target="_blank" class="tweet-retweet" href="https://twitter.com/intent/retweet?tweet_id='.$ai_tweets[$i]->id_str.'">retweet</a>';

				$ai_favorite_html='<a target="_blank" class="tweet-favotite" href="https://twitter.com/intent/favorite?tweet_id='.$ai_tweets[$i]->id_str.'">favorite</a>';
				...
				$ai_output.='<div class="ai-twitter-item" id="'.$ai_tweets[$i]->id_str.'">'.
				'<div class="imgdisplay">'.
					'<div class="avatarimg">'.$ai_img_html.'</div>
						<div class="tweettxts">
							<div class="tweettext"><div class="authorinfo">'.$ai_username_html.'</div><div class="tweet-posted-date">'.$ai_timestamp_html.'</div>'.IMTConvertLinks($ai_tweets_text).'</div> 
							<div class="tweetlink">
								'.$ai_replay_html.'
								'.$ai_retweet_html.'
								'.$ai_favorite_html.'
		...
		$ai_loadmore_html ='';
		if(empty($ai_tweets->errors)) {
			$ai_loadmore_html.="<a href='javascript:void(0);' id='ai-more' class='ai-more'><span>".$ai_load_more_text."</span></a>";
		}
		$ai_output.=$ai_loadmore_html."</div></div>";
		$ai_output.="</div>";
	} else {
...
class AI_Twitter_Widget extends WP_Widget {
	...
	public function form($instance) {
		...
		echo "<a href='".admin_url()."options-general.php?page=ai-plugin'>More Settings</a><br><br>";
	}
}
...
function ai_frontendloadjs() {
	 ...
	 wp_enqueue_script( 'ai_scripts', plugins_url( '/js/ai-twitter.js', __FILE__ ), array( 'jquery' ), '2.4', true );
			...
			if(!empty($ai_tweets->errors)) {
				...
				$ai_output.='<div class="ai-twitter-item" id="'.$ai_tweets[$i]->id_str.'">'.
					'<div class="imgdisplay">'.
						'<div class="avatarimg">'.$ai_img_html.'</div>
						<div class="tweettxts">
							<div class="tweettext"><div class="authorinfo">'.$ai_username_html.'</div>'.IMTConvertLinks($ai_tweets_text).'&nbsp</div> 
							<div class="tweetlink">
								'.$ai_replay_html.'
								'.$ai_retweet_html.'
								'.$ai_favorite_html.'
